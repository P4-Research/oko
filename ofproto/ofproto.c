...
handle_bpf_show_prog(struct ofconn *ofconn, const struct ofp_header *oh)
{
    ...
    VLOG_INFO("Received BPF_SHOW_PROG request, Prog: %d", msg.prog_id);
    ovs_be16 nb_progs = 0;
    ...
    if (msg.prog_id != OFPBPF_ALL) {
        ...
        VLOG_INFO("Dumping all BPF programs");
        struct ubpf_vm *vm = NULL;
        HMAP_FOR_EACH (vm, hmap_node, &ofproto->ubpf_vms) {
            VLOG_INFO("Putting BPF program, %d", vm->prog_id);
            put_bpf_show_prog_info(buf, vm);
...
//    struct ofpbuf *output_buffer = ofputil_encode_bpf_show_prog_reply(&msg, oh, nb_progs);
