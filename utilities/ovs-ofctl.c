...
static void
ofctl_load_bpf_prog(struct ovs_cmdl_context *ctx)
{
    struct ofpbuf *request;
    struct vconn *vconn;
    enum ofputil_protocol protocol;
    ...
    const ovs_be16 prog_id = atoi(ctx->argv[2]);
    ...
    protocol = open_vconn_for_flow_mod(bridge, &vconn, usable_protocols);
    version = ofputil_protocol_to_ofp_version(protocol);
    request = ofputil_encode_bpf_load_prog(version, prog_id, program,
                                              length);
...
static void
ofctl_show_bpf_prog(struct ovs_cmdl_context *ctx)
{
    struct ofpbuf *request;
    struct ofpbuf *reply;
    struct vconn *vconn;
...
static void
ofctl_unload_bpf_prog(struct ovs_cmdl_context *ctx)
{
    struct ofpbuf *request;
    struct vconn *vconn;
    enum ofputil_protocol protocol;
    ...
    protocol = open_vconn_for_flow_mod(bridge, &vconn, usable_protocols);
    version = ofputil_protocol_to_ofp_version(protocol);
    request = ofputil_encode_bpf_unload_prog(version, prog);
...
static void
ofctl_update_bpf_map(struct ovs_cmdl_context *ctx)
{
    struct ofpbuf *request;
    struct vconn *vconn;
    ...
    request = ofputil_encode_bpf_update_map(version, prog, map,
                                            key, value, key_size,
                                            value_size, 1);
...
static const struct ovs_cmdl_command all_commands[] = {
    ...
    { "unload-bpf-prog", "switch prog", 2, 2, ofctl_unload_bpf_prog, OVS_RW },
    { "show-bpf-prog", "switch", 1, 2, ofctl_show_bpf_prog, OVS_RW },
